"""tr_resale API 雛形（dict 応答・専用スキーマは後から分離可）。"""
from __future__ import annotations

from decimal import Decimal

from fastapi import APIRouter, Depends
from sqlalchemy.orm import Session

from app.db.deps import get_session
from app.entities.tr_resale.model import TrResale
from app.entities.tr_resale.repository import TrResaleRepository

router = APIRouter(tags=["tr_resale"])


def _cell(v: object) -> object:
    if v is None:
        return None
    if hasattr(v, "isoformat"):
        return v.isoformat()
    if isinstance(v, Decimal):
        return float(v)
    return v


@router.get("/tr_resale/", response_model=list[dict])
def list_tr_resale(session: Session = Depends(get_session)) -> list[dict]:
    rows = TrResaleRepository.list_all(session)
    keys = [c.key for c in TrResale.__table__.columns]
    return [{k: _cell(getattr(r, k)) for k in keys} for r in rows]
